package org.jean.java_md5;

/**
 * MD5 implementation on Java Language
 * 
 * RFC: https://tools.ietf.org/html/rfc6151 WIKI:
...
public class MD5 {
	...
	private static int[] SHIFT_AMTS = new int[] { 7, 12, 17, 22, 5, 9, 14, 20, 4, 11, 16, 23, 6, 10, 15, 21 };
	...
	public static byte[] computeMD5(byte[] message) {
		...
		System.out.println("Total number of 512-bit 64-bytes (16 word) blocks: " + numBlocks);
		...
		System.out.println("Initializing A=" + toHexString(a) + " B=" + toHexString(b) + " C=" + toHexString(c) + " D="
				+ toHexString(d));
		...
		for (int i = 0; i < numBlocks; i++) {
			...
			int originalC = c;
			int originalD = d;
			System.out.println("  Before all rounds: A=" + toHexString(a) + " B=" + toHexString(b) + " C="
					+ toHexString(c) + " D=" + toHexString(d) + "; saving as AA/BB/CC/DD");
			for (int j = 0; j < 64; j++) {
				int mod16 = j & 0x0F;
				int div16 = j >>> 4;
				int f = 0;
				...
				System.out.println("    Applying [" + "BCDABCD".substring(3 - (j & 3), 7 - (j & 3)) + " "
						+ padTwo(bufferIndex) + " " + padTwo(SHIFT_AMTS[(div16 << 2) | (j & 3)]) + " " + padTwo(j + 1)
						+ "]: A=" + toHexString(debugVars[(j + 1) & 3]) + " B=" + toHexString(debugVars[(j + 2) & 3])
						+ " C=" + toHexString(debugVars[(j + 3) & 3]) + " D=" + toHexString(debugVars[j & 3]) + " T["
						+ j + "]=" + toHexString(TABLE_T[j]));
				if (((j + 1) & 3) == 0) {
					System.out.println("");	
				}
			}
			...
			c += originalC;
			d += originalD;
			System.out.println("After adding original values (AA/BB/CC/DD): A=" + toHexString(a) + " B="
					+ toHexString(b) + " C=" + toHexString(c) + " D=" + toHexString(d));
		}
